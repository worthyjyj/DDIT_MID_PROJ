...
function deletePick1(p_name,mb_no){
...
function deletePick2(p_name,mb_no){
...
function insertPick1(p_name,mb_no){
...
function insertPick2(p_name,mb_no){
...
$(function(){
	...
	success: function(res){
		if(res == "중복"){ //이미 찜하기 되어 있으면 
			deletePick1("<%=PNAME%>",mb_no); //프로필이름, 게시물 넘버
		}else if(res == "중복아님"){ // 찜하기 안되어 있으면 
			insertPick1("<%=PNAME%>",mb_no); //프로필이름, 게시물 넘버
		}
		 }
      })
	});
	...
	success: function(res){
		if(res == "중복"){ //이미 찜하기 되어 있으면 
			deletePick2("<%=PNAME%>",mb_no); //프로필이름, 게시물 넘버
		}else if(res == "중복아님"){ // 찜하기 안되어 있으면 
			insertPick2("<%=PNAME%>",mb_no); //프로필이름, 게시물 넘버
		}
		 }
      })
	});
  ...
  <a class="nav-link" href="<%=request.getContextPath()%>/noticeBoardList.do" style="margin-left: 680px; margin-right:none; color:black; font-size: 1.0em;">공지사항</a>
	...
	<section>
		<ul class="nav nav-tabs" style="width:1300px; margin-left:auto; margin-right: auto;">
			...
			<li class="nav-item"><a class="nav-link" href="#" id="regtab">등록순</a></li>
			<li class="nav-item"><a class="nav-link" href="#" id="hittab">조회수순</a></li>
		</ul>
	</section><br>
