...
function friends() {
	var room = "<%=room%>";
	$.ajax({
		url: "<%=request.getContextPath()%>/withMeFriends.do",
		data: { "room" : room },
		type: 'get',
		dataType: 'json',
		success: function(res) {
			...
			var code = "<div id='fp_div'>";
			$.each(res, function(i, v) {
				if(v.p_name != pName){
					code += "<div class='profile_friends'>"
					if(v.p_pic == null){
						code += '<img class="prof_img" src="https://tripsoda.com/images/common/profile/vector_profile_willy.svg">';
					}else{
						code += "<img class='prof_img' src='<%=request.getContextPath() %>/freeboard/imageView2.do?p_pic=" + v.p_pic + "'>";
					}
					code += "<br><p style='text-align:center;'>" + v.p_name + "</p><hr>";
					code += "<p>성별 : " + v.p_gen + "</p><hr>";
					code += "<p>나이 : " + v.p_age + "세</p>";
					code += "</div>";
				}
			});
			code += "</div>";
			
			$('#friendsDiv').append(code);
		}
	});
}
</script>
