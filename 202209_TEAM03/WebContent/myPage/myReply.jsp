...
$(function(){
	...
	$('#select_value').change(function(){
		location.href="<%=request.getContextPath()%>/myReply.do?num="+$("#select_value").val();
	})
})
</script>
</head>
							...
							<div class="container">
									  <table class="table table-bordered" id="hyomi" style="text-align: center; vertical-align: bottom;">
									    ...
									    <tbody id="noteTable">
											<%
									    		if(Integer.parseInt(num)==1){
									    			List<ReviewReplyVO> list=(List<ReviewReplyVO>)request.getAttribute("list");
									    			for(int i = 0; i < list.size(); i++){
									    				ReviewReplyVO noteVO = list.get(i);
									    				
									    				String content=noteVO.getRcom_content();
									    				
									    				if(content.length()>10){
									    					content=(noteVO.getRcom_content()).substring(0, 10)+"...";
									    				}
									    	%>
									    		<tr>
									    		
									    			<td><input type="checkbox" name="checkbox12" id="comment<%=noteVO.getRcom_no()%>" value="<%=noteVO.getRcom_no()%>"></td>
									    		    <td><%=noteVO.getRev_no()%></td>
									    			<td><a class="detailNote" href="#" data-no="<%=noteVO.getRev_no() %>" data-num="<%=num%>"><%=content %></a></td>
									    			<td><%=noteVO.getRcom_date()%></td>
									    	...
									    	<%
									    			...
									    			List<FreeReplyVO> list=(List<FreeReplyVO>)request.getAttribute("list");
									    			for(int i = 0; i < list.size(); i++){
									    			FreeReplyVO noteVO = list.get(i);
									    			
									    			String content=noteVO.getFcom_content();
								    				
								    				if(content.length()>10){
								    					content=(noteVO.getFcom_content()).substring(0, 10)+"...";
								    				}
									    	%>
									    		<tr>
									    			<td><input type="checkbox" name="checkbox12" id="comment<%=noteVO.getFcom_no()%>" value="<%=noteVO.getFcom_no()%>"></td>
									    			<td><%=noteVO.getFb_no()%></td>
									    			<td><a class="detailNote" href="#" data-no="<%=noteVO.getFb_no() %>" data-num="<%=num%>"><%=content %></a></td>
									    			<td><%=noteVO.getFcom_date()%></td>
									    		</tr>
									    	<%
									    		}
									    		}else if(Integer.parseInt(num)==2){
									    			List<MainReplyVO> list=(List<MainReplyVO>)request.getAttribute("list");
									    			for(int i = 0; i < list.size(); i++){
									    				MainReplyVO noteVO = list.get(i);
									    				
														String content=noteVO.getMcom_content();
									    				
									    				if(content.length()>10){
									    					content=(noteVO.getMcom_content()).substring(0, 10)+"...";
									    				}
									    		%>
									    		<tr>
									    			<td><input type="checkbox" name="checkbox12" id="comment<%=noteVO.getMcom_no()%>" value="<%=noteVO.getMcom_no()%>"></td>
									    			<td><%=noteVO.getMb_no()%></td>
									    			<td><a class="detailNote" href="#" data-no="<%=noteVO.getMb_no() %>" data-num="<%=num%>"><%=content %></a></td>
									    			<td><%=noteVO.getMcom_date()%></td>
									    		</tr>
									    	<%}}%>
									    </tbody>
									  </table>
